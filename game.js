...
// Spell Configuration Definitions for All 6 Elements
const ElementSpells = {
  fire: {
...
class GameEngine {
  ...
  rebuildSpellSlotsUI(element) {
    ...
    for (let slotKey = 1; slotKey <= 5; slotKey++) {
      ...
      if (tooltipEl) {
        ...
        tooltipEl.querySelector('h4').style.color = elementColors[element];
  ...
  startBotMode() {
    ...
    const botColors = ['#ff007b', '#ffb700', '#39ff14', '#9d00ff', '#0077ff', '#e0e0e0'];
    const botElements = ['fire', 'ice', 'lightning', 'earth', 'water', 'wind'];
  ...
  updateBotAI() {
    ...
    // 1. Dodging incoming projectiles
    let dodgeX = 0;
    let dodgeY = 0;
    ...
    if (!isProjectilesClose) {
      ...
      const dLength = Math.hypot(dodgeX, dodgeY);
      moveX = dodgeX / dLength;
      moveY = dodgeY / dLength;
    }

    bot.vx = moveX * bot.speed;
    bot.vy = moveY * bot.speed;

    // 3. Spellcasting heuristics
    const botSpells = ElementSpells[bot.element];

    // Potions heal
    if (bot.health < 45 && bot.potionsLeft > 0 && bot.cooldowns[6] === 0) {
      this.castSpellAuthoritative('client', 6, { x: bot.x, y: bot.y });
    }
    // Shield Defense
    else if (isProjectilesClose && bot.cooldowns[5] === 0 && bot.mana >= botSpells[5].manaCost && bot.shield <= 0) {
      this.castSpellAuthoritative('client', 5, { x: bot.x, y: bot.y });
    }
    // Spawn Crate wall to block opponent if they are active
    else if (bot.wallsLeft > 0 && bot.cooldowns[7] === 0 && Math.random() < 0.008) {
      this.castSpellAuthoritative('client', 7, { x: opponent.x, y: opponent.y });
    }
    // High-tier attack
    else if (bot.cooldowns[4] === 0 && bot.mana >= botSpells[4].manaCost && Math.random() < 0.01) {
      this.castSpellAuthoritative('client', 4, { x: opponent.x, y: opponent.y });
    }
    // Medium-tier targeted/stun attacks
    else if (bot.cooldowns[3] === 0 && bot.mana >= botSpells[3].manaCost && Math.random() < 0.015) {
    ...
    // Basic projectile attacks
    else if (bot.cooldowns[2] === 0 && bot.mana >= botSpells[2].manaCost && Math.random() < 0.015) {
      this.castSpellAuthoritative('client', 2, { x: opponent.x, y: opponent.y });
    }
    else if (bot.cooldowns[1] === 0 && bot.mana >= botSpells[1].manaCost && Math.random() < 0.025) {
      const inaccuracy = (Math.random() - 0.5) * 45;
      this.castSpellAuthoritative('client', 1, { x: opponent.x + inaccuracy, y: opponent.y + inaccuracy });
  ...
  updatePhysicsAuthoritative() {
    ...
    for (let i = this.projectiles.length - 1; i >= 0; i--) {
      ...
      let hitTarget = null;
      if (proj.owner === 'host' && Math.hypot(proj.x - client.x, proj.y - client.y) < client.radius + proj.radius) {
        hitTarget = 'client';
  ...
  drawGame() {
    ...
    // 1. Draw Summoning Field background
    this.drawArenaField();

    // 2. Crate Wall Blocks
    this.drawCrateBlocks();

    // 3. Thunderbolt Indicators
    this.drawThunderboltWarnings();

    // 4. Spells Projectiles
    this.drawProjectiles();

    // 5. Wizard avatars (with hit flashes)
    this.drawWizardEntity(this.players.host);
    this.drawWizardEntity(this.players.client);

    // 6. Solid Jagged Lightning columns
    this.drawLightningBolts();

    // 7. Particles & shockwave rings
  ...
  syncHUDBarIndicators() {
    ...
    document.getElementById('p1-health-bar').style.width = `${Math.max(0, host.health)}%`;
    document.getElementById('p1-health-text').textContent = `${Math.round(host.health)} / 100`;
    ...
    document.getElementById('p2-health-bar').style.width = `${Math.max(0, client.health)}%`;
    document.getElementById('p2-health-text').textContent = `${Math.round(client.health)} / 100`;
