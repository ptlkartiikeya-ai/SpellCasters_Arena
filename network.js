...
class NetworkManager {
  ...
  getCustomization() {
    ...
    return {
      name: this.nameInput.value.trim() || 'Spellcaster',
      color: activeColorBtn ? activeColorBtn.getAttribute('data-color') : '#00f3ff',
      element: activeElementBtn ? activeElementBtn.getAttribute('data-element') : 'fire',
  ...
  saveCustomization() {
    ...
    const config = this.getCustomization();
    localStorage.setItem('wizard_duel_customization', JSON.stringify(config));
  }
  ...
  loadCustomization() {
    try {
      ...
      if (data) {
        ...
        if (config.element) {
          this.elementButtons.forEach(btn => {
            if (btn.getAttribute('data-element') === config.element) {
              this.elementButtons.forEach(b => b.classList.remove('active'));
              btn.classList.add('active');
            }
          });
